...
public class TrimBinaryTree {
	public TreeNode trimBinarySearchTree(TreeNode root, int L, int R) {
		if (root == null) {
			return root;
		}
		if (root.val >= L && root.val <= R) {
			root.left = trimBinarySearchTree(root.left, L, R);
			root.right = trimBinarySearchTree(root.right, L, R);
		} else if (root.val < L) {
			root = trimBinarySearchTree(root.right, L, R);
		} else {
			root = trimBinarySearchTree(root.left, L, R);
		}
		return root;
	}
